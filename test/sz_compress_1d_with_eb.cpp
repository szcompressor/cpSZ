...
int main(int argc, char ** argv){
    ...
    int r1 = atoi(argv[3]);
    bool quantize_eb = atoi(argv[4]);
    double base_eb = 1e-9;
    if(argc > 6) base_eb = atof(argv[5]);
    double global_eb = 0;
    if(argc > 7) global_eb = atof(argv[6]);
    ...
    float * ub = (float *) malloc(r1*sizeof(float));
    float * lb = (float *) malloc(r1*sizeof(float));
    for(int i=0; i<num_elements; i++){
        ub[i] = 100;
        lb[i] = -100;
    }

    // float * ub = readfile<float>(argv[7], num_elements);
    // float * lb = readfile<float>(argv[8], num_elements);
    ...
    unsigned char * result = sz_compress_1d_with_eb(data, eb, r1, result_size);
    ...
    float * dec_data = sz_decompress_1d_with_eb<float>(result, eb, r1);
    err = clock_gettime(CLOCK_REALTIME, &end);
