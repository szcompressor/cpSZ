...
int main(int argc, char ** argv){
    ...
    int r2 = atoi(argv[4]);
    int r3 = atoi(argv[5]);
    bool quantize_eb = atoi(argv[6]);
    double base_eb = 1e-9;
    if(argc > 7) base_eb = atof(argv[7]);
    double global_eb = 0;
    if(argc > 8) global_eb = atof(argv[8]);
    ...
    float * ub = (float *) malloc(num_elements*sizeof(float));
    float * lb = (float *) malloc(num_elements*sizeof(float));
    for(int i=0; i<num_elements; i++){
        ub[i] = 100;
        lb[i] = -100;
    }

    // float * ub = readfile<float>(argv[9], num_elements);
    // float * lb = readfile<float>(argv[10], num_elements);
    ...
    unsigned char * result = sz_compress_3d_with_eb(data, eb, r1, r2, r3, result_size);
    ...
    float * dec_data = sz_decompress_3d_with_eb<float>(result, eb, r1, r2, r3);
    err = clock_gettime(CLOCK_REALTIME, &end);
